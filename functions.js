...
exports.tuba_player_privacy = (req, res) => {
  let now = moment();
  res.render('tuba_player_privacy', {
    title: 'tuba_player_privacy',
    user: req.session.user,
  });
  console.log(
    req.ip + ' ' + now.format('DD/MM/YYYY HH:mm:ss') + ' tuba_player_privacy'
  );
};
...
exports.profile = (req, res) => {
  ...
  let data = {};
  data.token = sign(
    { username: req.session.user, email: req.session.email },
    process.env.JWT_KEY
  );
  ...
  if (req.query.id) {
    mongo.existId.bind(req.db)(req.query.id, (record) => {
      if (record) {
        ...
        mongo.updateRecord.bind(req.db)(
          'users',
          { _id: record._id },
          record,
          (resp) => {
    ...
    mongo.setEmail.bind(req.db)(
      { user: req.session.user, email: req.query.email },
      (resp) => {
        if (resp) {
          otherEmail = true;
...
exports.player = (req, res) => {
  let dir =
    __dirname +
    '/public/users/' +
    (req.params.user ? req.params.user : req.session.user);
  let date = new Date();
  let data = {};
  data.token = 'none';
  let visit = {
...
exports.songs = async (req, res) => {
  ...
  if (!fs.existsSync(dir)) {
    res
      .status(404)
      .send(`User ${req.me.username} has no songs stored in the server!`);
  } else {
    fs.readdir(dir, (err, files) => {
  ...
  mongo.auth.bind(req.db)(req.body.username, req.body.password, (user) => {
    if (user) {
      ...
      res
        ...
        .json({ ok: true, token: token, data: user });
    } else {
      ...
      mongo.addUser.bind(req.db)(
        ...
        (success) => {
          if (success) {
            ...
            res
              .header('auth-token', token)
              .json({ ok: true, token: token, data: data });
          } else {
...
exports.audio = async (req, res) => {
  ...
  if (data.username === req.params.user) {
    ...
    if (dirSize > (data.permission || 1)) {
      res.json({
        ok: false,
        msg: `You've reached your limit of ${data.permission || 1}GB.`,
      });
    } else {
